...
const ENEMY_SCENES: Dictionary = {
	"SARS": preload("res://Characters/Enemy/sars/sars.tscn")
}
...
func _spawn_enemies() -> void:
	for enemy_position in enemy_positions_container.get_children():
		var enemy: CharacterBody2D = ENEMY_SCENES.SARS.instantiate()
...
func _process(delta):
	if entered1 == true:
		...
		if Input.is_action_just_pressed("ui_accept"):
			get_tree().change_scene_to_file("res://Rooms/endroom.tscn")
